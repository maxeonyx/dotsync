...
**Deleting needs the remote**, which makes it the second command that does, for `init`'s reason: the deletion *is* the remote not having the branch, so there is no local half to defer and converge later. The remote goes first and this machine's bookmark goes in the same operation, which is what makes both halves of it recoverable. A remote that will not take it — refusing the write, or not there at all — is a stop that changed nothing, so the way out is to run the same command again. A run that dies in between is a machine whose next fetch finds the branch gone and removes the bookmark itself, like any other machine.
