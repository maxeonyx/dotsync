...
- **Six red tests for the symlink decision are committed pending (2026-08-13); nothing is implemented.** In `tests/user_flows.rs`: `a_symlink_on_a_scope_materialises_in_home_as_a_symlink` (T1, the sharpest — reachable today without lifting the commit refusal, via a foreign git client plus a cascade), `a_sync_replaces_a_home_symlink_instead_of_writing_through_it` (T2, the data loss), `commit_records_a_symlink_as_a_symlink` (T3, lifts the refusal), `a_symlink_to_a_sibling_script_survives_the_round_trip_to_another_machine` (T4, the end-to-end story), `status_and_diff_report_a_symlink_where_a_regular_file_is_expected` (T5), and `a_symlink_to_home_records_one_entry_rather_than_sweeping_home` (T6 — a regression guard on a real leak, because today's blanket refusal is the only thing closing it and T3 removes that). Implementing T3/T6 will break three tests that currently assert the refusal: `a_symlink_pointing_at_home_cannot_be_used_to_sweep_it`, `a_symlinked_selection_path_is_refused_whether_it_is_a_file_or_a_directory`, and `a_symlink_under_a_named_directory_is_reported_rather_than_silently_skipped`. They encode the placeholder policy, not the decided one; expect to rewrite or delete them in the same commit.

- **The Windows half of the symlink decision has no test, because the commit path cannot know what a Windows scope is.** The scope graph is `name -> parents` and nothing else (`scope_graph.rs`); `os_scope` exists only in `detect_machine`, from `DOTSYNC_OS`, and is used once at init to decide which scope to create. So a Linux machine committing a link to `all` has no way to ask whether any machine below `all` is a Windows machine. Rejecting symlinks on Windows scopes needs the scope graph to carry a platform — `init` recording something like `os = "windows"` on the OS scope it creates, and `commit` checking the target scope's descendants for it — which is a config-schema change, not a guard. Not invented here.
