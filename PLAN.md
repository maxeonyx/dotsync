...
- **Commit selection refuses every symlink, deliberately and conservatively — and there is an open question for Max behind it.** A selection path that resolves through a link is refused whether the link is the last component or one above it, and whether it points inside home or out of it. That is what closed the sweep found in re-gate: `ln -s $HOME $HOME/selflink; dotsync commit all -m msg -- selflink/` published 72 files including `.ssh/id_ed25519`, `.netrc`, `sync-state.json` and the whole hidden `.jj` store, exit 0. But symlinked config — `~/.config/nvim` pointing at a checkout elsewhere — is a real pattern that people really use, and refusing it is a decision rather than a discovery. **Open question: should dotsync ever knowingly follow a symlink — recording the target's content under the link's path, and syncing back through the link — or should such config be moved into home first? Until that is answered, commit selection refuses them.** This affects selection only: `sync` is untouched, and a scope that already holds a symlink is written to home as jj has always written it.
