...
- For the current version, run `dotsync --version` or check the releases; don't record it here, it rots.
- Live fleet: remote `git@github.com:maxeonyx/dotfiles.git`, twelve scopes, `all` → `home`/`work`/`linux`/`windows` → intersections → machines (`mc-wsl-fd`, `mx-xps-cy`, `maxeonyx-pc-windows`, and `mx-vps-fd`, a machine that no longer exists — nothing deletes a scope yet, see "Scope graph changes beyond creation").
